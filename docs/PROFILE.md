...
- Decode tok/s: [results/decode_table.json](../results/decode_table.json) (Issue 9 / #86, 2026-09-07; protocol [DECODE.md](DECODE.md)). Cite the JSON. **Do not average protocols:** that table (concurrency-1 MTP-off `generation_tok_s_mean` 18.43–19.01) is not the old solo MTP anecdote (~60.9 / ~86 in [PROVENANCE.md](PROVENANCE.md)) and not prefill walls (dual fill 483.62 / 488.35 s). Not 8-token dummy walls. Not MTP short-load. #64 solo+short8 archive: [results/decode_table_issue64.json](../results/decode_table_issue64.json).
